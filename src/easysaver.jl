...
function getSaver(foldername::String)
    ...
    function saver(var::Union{Symbol, Expr}, mark=nothing; expl="", textable=true)
        ...
        if eval(:(typeof($var)))==Figure
            ...
            if textable && sizeof(eval(var))<50000
                thedata = repr(eval(var))
            else
                thedata = "--[Saved data with size: $(sizeof(eval(var)))]--"
            end
            
            saved_list[varname] = eval(var)
            saved_text[varname] = "\n  $expl\n$thedata\n - [saved: $(now())]  - ::$(eval(:(typeof($var))))\n"
        end
...
# saveme(:asd,:large)
# # saved large data...
# # other than string, issue proposed
...
# saveme(:asd, textable=false)

# for i in 1:10
#     saveme(:(join(asd, "_")), i, expl="Array repeatly saved in round $i")
#     # Array as data saved...
# end
